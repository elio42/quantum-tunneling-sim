...
{ // int argc, char *argv[]
    ...
    parser.add_argument("-b", "--boundary-potential")
        .help("Potential at the boundaries of the grid, controls how much waves are reflected at the boundaries. Should be large enough to avoid quantum tunneling through the boundaries.")
        .default_value(1e6)
        .scan<'g', double>()
        .nargs(1);
    
    parser.add_argument("-s", "--sigma")
        .help("Width of the Gaussian wave packet, make sure it fits well within the grid.")
        .default_value(0.25)
        .scan<'g', double>()
        .nargs(1);

    parser.add_argument("--tunneling-pot")
        .help("define the pot of the tunneling wall")
        .default_value(1e3)
        .scan<'g', double>()
        .nargs(1);
    
    parser.add_argument("--tunneling-wall-disabled")
        .help("disable the tunneling wall in the enviroment potential")
        .default_value(false)
        .flag();

    parser.add_argument("--time-code")
        .help("print the total time taken by the simulation and do not save any output files")
        .default_value(false)
        .flag();
    ...
    bool time_code = parser.get<bool>("--time-code");

    // Run The Simulation.
    SSFM1D ssfm(parser.get<double>("--number-of-points"), parser.get<double>("--length"), parser.get<size_t>("--padding"), parser.get<double>("--delta-time"), parser.get<int>("--number-of-waves"), parser.get<double>("--boundary-potential"), parser.get<double>("--sigma"), parser.get<double> ("--tunneling-pot"), !parser.get<bool>("--tunneling-wall-disabled"));
    ...
    if (!time_code) {
        ...
        ssfm.output_enviroment_potential(output_dir);
    }
    ...
    while (current_time < final_time) {
        if (!time_code && current_time >= next_output_time){
