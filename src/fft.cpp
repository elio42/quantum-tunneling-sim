...
vector<complex<double>> FFT::forward(const vector<complex<double>>& input) {
    ...
    return forward_recursive(input); // Placeholder return value
}

vector<complex<double>> FFT::forward_recursive(const vector<complex<double>>& input) {
    const size_t n = input.size();
...
vector<complex<double>> FFT::inverse(const vector<complex<double>>& input) {
    const size_t n = input.size();
    ...
    return inverse_recursive(input); // Placeholder return value
}

vector<complex<double>> FFT::inverse_recursive(const vector<complex<double>>& input) {
    const size_t n = input.size();
    if (n == 1) {
        return input;
    }
    ...
    for(size_t i = 0; i < n/2; ++i) {
        ...
        out[i] = (even[i] + odd[i] * twiddle_factor(i, -n))/complex<double>(n);

        out[i + n/2] = (even[i] - odd[i] * twiddle_factor(i, -n))/complex<double>(n);
